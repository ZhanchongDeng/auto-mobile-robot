...
depthAndBeaconJac = @(x) [HBeacont(x, sensor_pos, beacon); HjacDepth(x, map, sensor_pos, n_rs_rays)];
h = @(x) [hBeacon(x,sensor_pos,beacon);depthPredict(x,map,sensor_pose,linspace(3*pi/20,-3*pi/20,k))];
...
%         turnAngle(CreatePort, W, -30);
    ...
    % Limit the commands
    [cmdV, cmdW] = limitCmds(cmdV, cmdW, 0.4, 0.13);
